...
public class ImageManipulationUtilsTests {
	...
	public void testResize() throws IOException {
		...
		BufferedImage targetImage = ImageIO.read(new File(DEFAULT_IMAGES_PATH + targetImageName + "." + ImageType.PNG.toString()));
		BufferedImage resizedImage = ImageManipulationUtils.resize(targetImage, targetImage.getWidth() / 2,
				targetImage.getHeight() / 2);
		ImageIO.write(resizedImage, ImageType.PNG.toString(), new File(TESTS_OUTPUT_PATH + targetImageName + "_halved." + ImageType.PNG.toString()));
	...
	public void testAverageColor() throws IOException {
		...
		BufferedImage image1 = ImageIO.read(new File(DEFAULT_IMAGES_PATH + targetImage1 + "." + ImageType.PNG.toString()));
		BufferedImage image2 = ImageIO.read(new File(DEFAULT_IMAGES_PATH + targetImage2 + "." + ImageType.PNG.toString()));
	...
	public void testCropImage() throws IOException {
		...
		ImageIO.write(resultingImage, ImageType.PNG.toString(), new File(TESTS_OUTPUT_PATH + imageName + "_forth_corner." + ImageType.PNG.toString()));
	...
	public void testReplaceAt() throws IOException {
		...
		BufferedImage image1 = ImageIO.read(new File(DEFAULT_IMAGES_PATH + image1Name + "." + ImageType.PNG.toString()));
		...
		ImageIO.write(resultImage, ImageType.PNG.toString(), new File(TESTS_OUTPUT_PATH + "hibridImage." + ImageType.PNG.toString()));
